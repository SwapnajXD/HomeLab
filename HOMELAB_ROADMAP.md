...
Status reflects the operator-supplied [September 12, 2026 rebuild record](rebuild-history.md), not a new live audit. Checked items are reported achievements; unchecked items remain planned. Earlier milestones retain their historical context.
...
These achievements do not imply automated backups, complete restore coverage, or current uptime guarantees. Apollo's incident and dynamic WAN-aware firewall/NAT resolution remain in [postmortems](docs/historical/postmortems.md).
...
The Dashboard API, custom Homepage data widgets, retired fetch pipelines, and LocalStack do not define the V2 core. See [retired services](docs/historical/retired-services.md) for the build/test/operate/evaluate/retire story; historical source and incident reports are preserved.
