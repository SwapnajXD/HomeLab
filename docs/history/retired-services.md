...
| Olympus Dashboard API | Retired from active deployment | Backend maintenance no longer justified the value; source retained in `archive/v1/docker-compose/dashboard-api/` |
...
| Old data-fetch scripts and associated cron/data-sync pipeline | Historical / retired from deployment | Retained `archive/v1/scripts/fetch_*.sh` work demonstrates integration and operational lessons |
| LocalStack | Deprecated | Previously superseded by Floci; both are absent from the reported V2 stack, with examples retained |
| [Pre-V2 roadmap](v1/roadmap-pre-v2.md) | Historical / superseded | Original milestones, abandoned plans and priorities preserved verbatim |
